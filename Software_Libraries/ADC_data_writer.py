...
class ADCDataFileWriter(IADCDataWriter):
    def __init__(self, file):
        ...
        #   add try catch?
        # TODO this should probably be done on a start recording function and saved and stopped on a stop recording
        #  function and we should have the ability to provide different filenames each time -- right now if one
        #  recording is started we can't start another one after the first one is stopped -- also we need the file name
    ...
    def write_ADC_data(self, sensor_data: SensorData, time_sample_was_taken: datetime):
        # Throw error if recording time has not been set yet
        if self.recording_start_time is None:
            raise Exception("Error, recording start time must be set before recording/writing data")
        ...
        elapsed_recording_time = time_sample_was_taken - self.recording_start_time

        self.file_writer.writerow(
            [elapsed_recording_time.total_seconds(), sensor_data.t0, sensor_data.p0, sensor_data.p1, sensor_data.p2, sensor_data.p3,
             sensor_data.p4])
